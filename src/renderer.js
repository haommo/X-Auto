...
function deleteProfile(profileName, userDataDir) {
  ...
  if (rowDeleted) {
    console.log(`Đã xóa profile thành công`);
  } else {
    console.log(`Không tìm thấy profile`);
  }
...
function deleteUserDataDir(dir) {
  ...
  if (fs.existsSync(decodedDir)) {
    ...
    console.log(`Thư mục dữ liệu ${decodedDir} đã bị xóa.`);
  } else {
    console.log(`Thư mục dữ liệu ${decodedDir} không tồn tại.`);
  }
}
...
async function runProfile(profile, settings) {
  ...
  try {
    ...
    if (isLoggedIn) {
      ...
      await handleLogin(page, profile);
...
async function navigateToPage(page, url) {
  try {
    ...
    throw new Error("Lỗi khi điều hướng đến link Post");
  }
}
...
async function handlePostLogin(page, linkPost, minTime, maxTime, profile) {
  try {
    ...
    try {
      await page
        .locator("span ::-p-text(Accept all cookies)")
        .setTimeout(3000)
        .click();
    } catch (error) {
      console.log(
        "No 'Accept all cookies'"
      );
    }

    const postContent = await getPostContent(page);
    if (!postContent) {
      throw new Error("Tìm kiếm nội dung bài post thất bại");
    }
    ...
    if (!tweetSuccess) {
      throw new Error("Click tweet thất bại");
    }
    await waitRandomTime(minTime, maxTime);
  } catch (error) {
    console.error(
...
async function getPostContent(page) {
  try {
    ...
    throw new Error("Lỗi khi lấy nội dung bài viết");
  }
}
...
async function getReplyTextFromOpenAI(apiKey, postContent) {
  try {
    ...
    throw new Error("Lỗi khi lấy nội dung trả lời từ OpenAI");
  }
}
...
async function enterReplyText(page, replyText) {
  try {
    ...
    throw new Error("Lỗi khi nhập nội dung trả lời");
  }
}

async function clickTweetButton(page) {
  try {
    // Tìm và nhấn nút tweet
    await page.locator('button[data-testid="tweetButtonInline"]').click();
    console.log("Đã click Tweet");

    // Chờ đợi phần tử <time> với nội dung là "Now" xuất hiện
    await page.locator('time ::-p-text("Now")').wait()
    console.log("Tweet đã được đăng thành công.");

    return true;
  } catch (error) {
    console.error(
      "Lỗi khi nhấn nút tweet",
      error
    );
    throw new Error("Lỗi khi nhấn nút tweet");
  }
}


async function handleLogin(page, profile) {
  try {
...
async function enterUsername(page, username) {
  try {
    ...
    throw new Error("Lỗi khi nhập username");
  }
}
...
async function clickNextButton(page) {
  try {
    ...
    throw new Error("Lỗi khi nhấn nút tiếp theo");
  }
}
...
async function enterPassword(page, password) {
  try {
    ...
    throw new Error("Lỗi khi nhập password");
  }
}
...
async function clickLoginButton(page) {
  try {
    ...
    throw new Error("Lỗi khi nhấn nút đăng nhập");
  }
}
...
async function enter2FACode(page, twoFA) {
  try {
    ...
    throw new Error("Lỗi khi nhập mã 2FA");
  }
}
...
async function get2FACode(twoFA) {
  try {
    ...
    throw new Error("Lỗi khi lấy mã 2FA");
  }
}
...
async function importExcelProfiles() {
  ...
  try {
    ...
    console.log("Nhập profile thành công.");

    await loadProfiles();
  } catch (error) {
    console.error("Lỗi khi open file Excel:", error);
  }
}
